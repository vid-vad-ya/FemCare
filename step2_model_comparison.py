...
# ============================================================
# This script:
#   A) Runs feature selection (same pipeline as Step 1)
#   B) Compares 6 classifiers on the final 15 features
#   C) Builds a Hybrid Voting Ensemble from the best models
#   D) Ablation study — removes top feature, tries symptom groups
# ============================================================
...
# XGBoost unavailable in sandbox — using GradientBoostingClassifier as equivalent

# ── 0. LOAD DATA ─────────────────────────────────────────────
DATA_PATH = "/mnt/user-data/uploads/dataset__1_.xlsx"
...
# ── 1. FEATURE SELECTION (same as Step 1) ────────────────────
...
# Identify top feature (highest AdaBoost importance among final 15)
...
# Pick top 3 models by CV-AUC for ensemble
...
# Run XGBoost on each group (XGBoost was usually the best or near-best)
print("\n\nAblation results using XGBoost:")
print(f"\n{'Feature Group':<35} {'# Feats':>7} {'CV-AUC':>8} {'Test-AUC':>9} {'F1':>7}")
print("-" * 70)

ablation_results = {}
xgb_base = GradientBoostingClassifier(n_estimators=100,
                              random_state=42)
...
ax.axhline(0.8, color='gray', linestyle='--', alpha=0.5, label='AUC=0.8 threshold')
...
plt.tight_layout()
plt.savefig('/home/claude/plot_A_model_comparison.png', dpi=150)
plt.close()
...
plt.tight_layout()
plt.savefig('/home/claude/plot_B_roc_curves.png', dpi=150)
plt.close()
print(">> Saved: plot_B_roc_curves.png")

# --- Plot 3: Radar / Spider chart for model metrics ---
...
plt.tight_layout()
plt.savefig('/home/claude/plot_C_radar.png', dpi=150)
plt.close()
...
abl_test  = [ablation_results[n]['test_auc'] for n in abl_names]
...
ax.axhline(base_cv, color='green', linestyle='--', lw=1.5, alpha=0.7, label=f'Baseline CV AUC ({base_cv:.3f})')
ax.set_xticks(x2)
...
plt.tight_layout()
plt.savefig('/home/claude/plot_D_ablation.png', dpi=150)
plt.close()
print(">> Saved: plot_D_ablation.png")

# --- Plot 5: Confusion Matrix — Best model vs Hybrid ---
...
plt.tight_layout()
plt.savefig('/home/claude/plot_E_confusion.png', dpi=150)
plt.close()
...
all_res = {**results}
sorted_all = sorted(all_res.items(), key=lambda x: x[1]['cv_auc'], reverse=True)
